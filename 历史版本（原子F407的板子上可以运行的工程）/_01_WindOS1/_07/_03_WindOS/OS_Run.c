...
void OSThread_Free(void)
{
	...
	{  
		...
		else 
		{
			...
			{
				if(FreeRun ->state == STATE_ACTIVE) //判断是否激活   
				{ 
					FreeRun ->AppFun(); //运行程序  
					FreeRun ->loading = FreeRun ->reload;		
				}
				else if(FreeRun ->state == STATE_ONE_OFF) //判断是一次性的程序  
				{ 
					FreeRun ->AppFun(); //运行程序   
				...
				{	
					FreeRun ->state = STATE_ACTIVE;//重新激活
					FreeRun ->loading = FreeRun ->reload;		 
				}  
...
void OSThread_Primary(void)
{
	...
	else
	{
	    ...
	    {
			if(PrimaryRun ->state == STATE_ACTIVE) //判断是否激活
			{ 
				PrimaryRun ->AppFun(); //运行程序    
				PrimaryRun ->loading = PrimaryRun->reload;
			}   
			else if(PrimaryRun ->state == STATE_ONE_OFF) //判断是一次性的程序
			{
				PrimaryRun ->AppFun(); //运行程序 
				OSTask_Remove(PrimaryRun ->AppFun);   
				 
			}  
			else if(PrimaryRun ->state == STATE_SUSPEND) //判断是一次性的程序
			{
...
void OSThread_System(void)
{
	...
	else
	{
	    ...
	    {
			if(SystemRun ->state == STATE_ACTIVE) //判断是否激活
			{ 
				SystemRun ->AppFun(); //运行程序    
				SystemRun ->loading = SystemRun->reload;
			}   
			else if(SystemRun ->state == STATE_ONE_OFF) //判断是一次性的程序
			{
				SystemRun ->AppFun(); //运行程序 
				OSTask_Remove(SystemRun ->AppFun);   
				 
			}  
			else if(SystemRun ->state == STATE_SUSPEND) //判断是一次性的程序
			{
				SystemRun ->state = STATE_ACTIVE; 	   
			}
