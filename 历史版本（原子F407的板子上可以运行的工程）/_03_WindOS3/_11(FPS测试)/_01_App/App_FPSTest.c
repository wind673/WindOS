...
{
	volatile u32 _cntTure,_cntImage,_NowTime,_InTime;
	u16 _color = 0;
	OS_BackColor_Set(GRAY);		
	
	/* 刷纯色图 -------------------------------------*/
	_InTime = TIM_GetCounter(TIM2); _cntTure = 0; 
	...
	OS_String_Show(FONT_3*1,FONT_3*2,(u8*)" True FPS:    ",FONT_2,1);
	OS_Num_Show(FONT_3*4,FONT_3*2,_cntTure,4,FONT_2,0);
...
Type_AppList App_Image = {NULL,NULL,FPSTest,NULL, \
