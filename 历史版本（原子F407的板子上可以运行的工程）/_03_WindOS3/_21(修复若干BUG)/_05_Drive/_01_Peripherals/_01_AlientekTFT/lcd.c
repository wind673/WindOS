...
//color:颜色 
//num:数值(0~4294967295);	 
void LCD_Show0Num(u16 x,u16 y,u32 num,u8 len,u8 size,u8 mode)
{         	
	u8 t,temp;
	u8 enshow=0;						   
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				LCD_ShowChar(x+(size/2)*t,y,'0',size,mode);
				continue;
			}else enshow=1; 
...
//digit:保留位数
//size:字体大小  12/16/24
void LCD_ShowDecim(u16 x,u16 y,double num,u8 digit,u8 len,u8 size,u8 mode)//长度 包括.在内
{ 
	...
	len=len-digit-1;
	temp=numd/digitnum;
	LCD_ShowxNum(x,y,temp,len,size,mode);//显示整数部分
	temp=(u32)(numd-((double)(numd/digitnum)*digitnum));
	Show_Str(x+len*(size/2),y,size,size,".",size,mode);
	LCD_Show0Num(x+(len+1)*(size/2),y,temp,digit,size,mode);//显示小数部分
