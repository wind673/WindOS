...
{  
	...
	/* CPU 显示 ------------------------------------*/  
	OS_String_Show(FONT_3*2,FONT_3*10,(u8*)"CPU:",FONT_3); 	
	OS_String_Show(FONT_3*7,FONT_3*10,(u8*)"%",FONT_3);  
	OS_Decim_Show(FONT_3*4,FONT_3*10,OS_CPU/100.0f,2,5,FONT_3);   
	Task_Show();			
...
{ 
	...
	OS_String_Show(FONT_1,0,(u8*)"%",FONT_1);  
	OS_Decim_Show(FONT_1,0,OS_CPU/100.0f,2,5,FONT_1);   
	//OSTask_Show();			
...
Type_AppList App_CPU_Show = {CPU_Show,CPU_Show_Back,CPU_Show_Start,CPU_Show_End, \
...
{ 			
	...
	while(1)			
	{
		...
		temp = temp ->next;		
		if(temp ->state == OS_STATE_SUSPEND)
		{
			...
			OS_String_Show(20,20+FONT_2*_line++,temp ->name,FONT_2);   			
		}
