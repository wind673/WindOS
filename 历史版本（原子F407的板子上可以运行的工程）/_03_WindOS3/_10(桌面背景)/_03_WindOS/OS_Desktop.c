...
{    
	...
	DeskSourceColor = OS_Point_Read(0,FONT_1+2);	
	return 0;	
}
//程序注册表 
...
{  
	 ...
	 if(OS_Point_Read(0,FONT_1+2) != DeskSourceColor) OS_Clear_Y(FONT_1,1,NULL); //清屏
...
{   
	...
	if(Button_X.state == Button_Down) 
	{   
		while(Button_X.state == Button_Down);//等待按钮抬起 
		
		if(TouchPoint._LastX > Button_X.x1&&TouchPoint._LastX < Button_X.x2&&TouchPoint._LastY>Button_X.y1&&TouchPoint._LastY<Button_X.y2) 		
		{	
	...
	if(Button_T.state == Button_Down) 
	{   
		while(Button_T.state == Button_Down);//等待按钮抬起 
		if(TouchPoint._LastX > Button_T.x1&&TouchPoint._LastX < Button_T.x2&&TouchPoint._LastY>Button_T.y1&&TouchPoint._LastY<Button_T.y2) 		
		{
			x1 = 0;x2 = OS_LCD_WHITH;  
			y1 = 0;y2 = OS_LCD_LENTH;    
			OS_Rect_Draw(0,0,OS_LCD_WHITH,OS_LCD_LENTH,1,BLACK);					
...
{  
	 ...
	 if(Taskbar.run_state == BAR_FIRST){ OS_Clear_Y(FONT_1,0,WHITE);Taskbar.run_state = BAR_CLOSE;} 
	 ...
	 {
		OSRunning_Break();
		Taskbar.dynamic_Y = OS_Clear_Y(TouchPoint._Y,0,WHITE); 
	 ...
	 else  
	 {
		...
		if(Taskbar.run_state == BAR_RUNNING)  		
		{
			...
			{			
				...
				OS_Clear_Y(OS_LCD_LENTH,0,WHITE);					
			}
			else 						
			{
