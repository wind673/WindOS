...
{  
	 ...
	 { 	
		...
		/* 记录滑动方向 (即时) -----------------------------------------*/ 
		TouchPoint._SlipDirection = 0x00; 
		...
		TouchPoint._LastState = TouchPoint._State;
		TouchPoint._LastSlipDirection =  TouchPoint._SlipDirection;	
		TouchPoint._LastBuffX[TouchPoint._LastCNT] = TouchPoint._BuffX[TouchPoint._CNT];	
		TouchPoint._LastBuffY[TouchPoint._LastCNT] = TouchPoint._BuffY[TouchPoint._CNT]; 
		  
		TouchPoint._CNT = (TouchPoint._CNT + 1)%TOUCH_BUFF_SIZE;
		TouchPoint._LastCNT = TouchPoint._CNT; 
