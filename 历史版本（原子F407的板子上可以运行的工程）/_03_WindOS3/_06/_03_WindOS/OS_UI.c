...
{ 	     
	 u16 x1,x2,y1,y2,ColorTemp = Color;	
	
	 if(state == 1) ColorTemp =(u16)~ColorTemp;			
	 if( OS_Point_Read(x,y) == ColorTemp) return; //屏幕变化才更新	 
	 
	 x1 = x; 		
	 y1 = y;		
	 x2 = x + strlen((const char*)str)*Fontsize/2;				
	 y2 = y + Fontsize;										
	  
	 OS_Rect_Draw(x1,y1,x2-1,y2-1,1,ColorTemp);   //内部    		 
	 
	 OS_Lines_Draw(x1,y2,x2,y2,~OS_Point_Read(x2+2,y2+2)); 		
	 OS_Lines_Draw(x2,y1,x2,y2,~OS_Point_Read(x2+2,y2+2)); 		
											
	 OS_TextColor_Set(~ColorTemp);		 			
	 OS_BackColor_Set(ColorTemp);					 
	 OS_String_Show(x1,y1,str,Fontsize); 		
