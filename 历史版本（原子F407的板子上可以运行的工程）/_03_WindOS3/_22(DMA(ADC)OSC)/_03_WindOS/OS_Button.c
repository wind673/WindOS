...
{ 	 
	 ...
	 if(pStruct == NULL) {OS_Error |= OS_ERROR_MALLOC;  return NULL;} //传递错误警告消息给系统
	 
	 while(Temp ->next != NULL)	//找到最后一个地址
	 {
	 	Temp = Temp ->next; 
	 } 
	 pStruct ->x1 = x; 				
...
{ 	 
	...
	{
		Temp = Temp ->next;
		if(Temp ->next == pStruct) break; //找到 pStruct 上一个按钮的地址
	} 
