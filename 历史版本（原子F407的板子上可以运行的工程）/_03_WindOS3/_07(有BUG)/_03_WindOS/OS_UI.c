...
{ 	     
	 ...
	 Type_Button *pStruct = (Type_Button*)malloc(sizeof(Type_Button));
...
{ 	   
	 ...
	 while(1)
	 {
		 if(ButtonStruct ->next != NULL)
		 {   
			 ButtonStruct = ButtonStruct ->next;
			 ColorTemp = ButtonStruct ->color;
			 	
			 if(ButtonStruct ->state == Button_Close) {ButtonStruct ->state = Button_Close_On; Taskbar_Draw(FONT_1);}
			 
			 if(ButtonStruct ->state == Button_Close_On) continue;
			 else if(ButtonStruct ->state == Button_Down) ColorTemp =(u16)~ColorTemp; //按钮被按下	
			 
			 if(OS_Point_Read(ButtonStruct ->x1,ButtonStruct ->y1) == ColorTemp) continue; //屏幕变化才更新	
			 ...
			 OS_TextColor_Set(~ButtonStruct ->color);		 			
			 OS_BackColor_Set(ButtonStruct ->color);					 
...
Type_AppList OS_Button_Show = {Button_Show,NULL,NULL,NULL, \
							   OS_STATE_ACTIVE_ON,OS_THREAD_3, \
							   (u8*)"Button_Show",10,NO_USE_OS_DESKTOP};			
