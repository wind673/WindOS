...
{ 	    
	...
	if(state == Button_Close) return;	 
	if(state == Button_Down)  ColorTemp =(u16)~ColorTemp; //按钮被按下	 
	...
	//按钮阴影   	 
	OS_Lines_Draw(ButtonStruct ->x1,ButtonStruct ->y2,ButtonStruct ->x2,ButtonStruct ->y2,~OS_Point_Read(ButtonStruct ->x2+2,ButtonStruct ->y2+2)); 	
				 
	OS_Lines_Draw(ButtonStruct ->x2,ButtonStruct ->y1,ButtonStruct ->x2,ButtonStruct ->y2,~OS_Point_Read(ButtonStruct ->x2+2,ButtonStruct ->y2+2)); 		
