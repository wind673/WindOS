...
unsigned int OSCPU_Show(void) 
{
	...
	tTextColor = TextColor_Set(OS_COLOR_CPU);
	tBackColor = BackColor_Set(OS_COLOR_STATUSBAR);	
	...
	/* CPU 显示 ------------------------------------*/  
	Show_String(0,0,(u8*)"CPU:",FONT_1); 
	Show_String(FONT_1*5,0,(u8*)"%",FONT_1);  
	LCD_ShowDecim(FONT_1*2,0,OS_CPU/100.0f,2,5,FONT_1);  
		
	
	BackColor_Set(tBackColor);	
	TextColor_Set(tTextColor); 
	//OSTask_Show();
	return 0;
...
unsigned int OSTask_Show(void) 
{
	...
	while(1)
	{
		...
		if(temp ->thread == OS_THREAD_FREE) TextColor_Set(OS_COLOR_FREE);   
		else if(temp ->thread == OS_THREAD_PRIMARY) TextColor_Set(OS_COLOR_PRIMARY);
		else if(temp ->thread == OS_THREAD_SYSTEM) TextColor_Set(OS_COLOR_SYSTEM); 
		else TextColor_Set(OS_COLOR_OTHER); 
