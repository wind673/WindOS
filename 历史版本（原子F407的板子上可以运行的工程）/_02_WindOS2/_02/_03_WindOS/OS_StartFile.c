...
{		 
	...
	TFT_Init(); 	Show_String(0,FONT_1*_line++,(u8*)"TFT_Init is OK!",FONT_1); 																
	RCC_Configuration();  Show_String(0,FONT_1*_line++,(u8*)"RCC_Configuration is OK!",FONT_1);   //初始化时钟	 
	My_GPIO_Init();	 Show_String(0,FONT_1*_line++,(u8*)"My_GPIO_Init is OK!",FONT_1); 
	TIM2_Config(); 	 Show_String(0,FONT_1*_line++,(u8*)"TIM2_Config is OK!",FONT_1); 
	TIM3_Config(); 	 Show_String(0,FONT_1*_line++,(u8*)"TIM3_Config is OK!",FONT_1); //主线程
	TIM4_Config();   Show_String(0,FONT_1*_line++,(u8*)"TIM4_Config is OK!",FONT_1); //系统线程
	tp_dev.init();   Show_String(0,FONT_1*_line++,(u8*)"LCD_Touch is OK!",FONT_1); //触摸屏初始化
