...
* @Explain        
* --------------------------------------------  
* x1,x2,y1,y2  坐标   str 语句   Color 按钮颜色    Fontsize 字体大小            
------------------------------------------------------------------------------*/ 
void OS_Button_Creat(u16 x1,u16 y1,u16 x2,u16 y2,u8 *str ,u16 Fontsize,u16 Color)    
{ 	     
	 u16 TempText,TempBack;
	 OS_Rect_Draw(x1,y1,x2,y2,1,Color);  //内部    					 
	 OS_Rect_Draw(x1,y1,x2,y2,0,BLACK);  //边框 
		 
	 TempText = OS_TextColor_Set(~Color);		 			
	 TempBack = OS_BackColor_Set(Color);					 
	 OS_String_Show(x1,y1,str,Fontsize); 		
	
	 
	 OS_TextColor_Set(TempText);
	 OS_BackColor_Set(TempBack);	
}						
